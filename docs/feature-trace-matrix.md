...
| Setup | `packages/client/pages/setup/index.vue` |
...
| AI | `packages/client/pages/ai/chat/index.vue`, `ai/dashboard.vue` |
| Settings | `packages/client/pages/settings/index.vue` |
| Gamification | `packages/client/pages/gamification/index.vue` |
...
|---|---|---|---|---|
| `/api/auth` (`auth.routes.ts`) | direct SQLite auth repository usage | `auth` | Settings/setup flows (`packages/client/pages/settings/index.vue`, `packages/client/pages/setup/index.vue`) | `packages/server/src/routes/core-routes.test.ts` |
...
| `/api/user` (`user.routes.ts`) | direct profile repository usage | `userProfile` | profile controls on `packages/client/pages/settings/index.vue` and user-related setup forms | `packages/server/src/routes/core-routes.test.ts` |
...
| `/api/studios` (`studio.routes.ts`) | direct repository access + optional `AIService` integrations | `studios`, `savedJobs` | `packages/client/pages/studios/index.vue`, `packages/client/pages/studios/[id].vue`, `packages/client/pages/studios/analytics/index.vue` (`useStudioAnalyticsPage` → `useAsyncData`) | `packages/server/src/routes/studio.test.ts` |
| `/api/resumes` (`resume.routes.ts`) | `resumeService`, `cvQuestionnaireService`, `AIService`, `exportService`, `gamificationService` | `resumes`, `settings`, AI chat/prompt inputs | `packages/client/pages/resume/index.vue`, `packages/client/pages/resume/build/index.vue`, `packages/client/pages/resume/preview/index.vue` | `packages/server/src/routes/resume.test.ts` |
| `/api/portfolio` (`portfolio.routes.ts`) | `portfolioService`, `exportService`, `gamificationService` | `portfolios`, `portfolio_projects` | `packages/client/pages/portfolio/index.vue`, `packages/client/pages/portfolio/preview/index.vue` | `packages/server/src/routes/portfolio.test.ts` |
...
| `/api/interview` (`interview.routes.ts`) | `interviewService` | `interview_sessions` | `packages/client/pages/interview/index.vue`, `packages/client/pages/interview/session/index.vue`, `packages/client/pages/interview/history/index.vue` | `packages/server/src/interview.test.ts` (server package root, not under `routes/`) |
| `/api/automation` (`automation.routes.ts`) | `applicationAutomationService`, `emailDeliveryService`, logger helpers, rate limiting + screenshot mapping | `automationRuns` | `packages/client/pages/automation/index.vue`, `packages/client/pages/automation/job-apply.vue`, `packages/client/pages/automation/email/index.vue`, `packages/client/pages/automation/scraper/index.vue`, `packages/client/pages/automation/runs/index.vue`, `packages/client/pages/automation/runs/[id].vue` | `packages/server/src/routes/automation.test.ts` |
...
| `/api/scraper` (`scraper.routes.ts`) | `scraperService` | `jobs`, `studios` via scraper upserts | `packages/client/pages/automation/scraper/index.vue` | `packages/server/src/routes/scraper.test.ts` |
| `/api/ai` (`ai.routes.ts`) | `AIService`, `contextManager`, `applicationAutomationService` | `chatHistory`, `jobs`, `resumes`, `settings`, `userProfile` | `packages/client/pages/ai/chat/index.vue`, `packages/client/pages/ai/dashboard/index.vue` | `packages/server/src/routes/ai.test.ts` |
| `/api/skills` (`skill-mapping.routes.ts`) | `skillMappingService`, `AIService` | `skillMappings`, `settings` | `packages/client/pages/skills/index.vue`, `packages/client/pages/skills/pathways/index.vue` | `packages/server/src/routes/skill-mapping.test.ts` |
| `/api/gamification` (`gamification.routes.ts`) | `gamificationService` | gamification tables/state | `packages/client/pages/gamification/index.vue` | `packages/server/src/routes/core-routes.test.ts` |
| `/api/settings` (`settings.routes.ts`) | settings + JSON export service hooks | `settings` | `packages/client/pages/settings/index.vue` | `packages/server/src/routes/settings.test.ts` |
