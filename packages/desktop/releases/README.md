...
- **Linux bundled server layout:** On Linux targets, `prepare-desktop-runtime.ts` ships `server/bao-desktop-server` as a POSIX shell launcher plus `server/bao-desktop-server.payload.gz` (gzip of the Bun-compiled binary) so linuxdeploy does not run `ldd` on the embedded server during AppImage creation. The manifest `serverExecutable` path is unchanged; Rust `Command::new` still launches that path.
