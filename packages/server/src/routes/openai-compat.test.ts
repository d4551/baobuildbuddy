...
import { OPENAI_COMPAT_ENDPOINTS } from "@bao/shared/constants/endpoints";
import { HTTP_STATUS_NOT_FOUND, HTTP_STATUS_OK } from "@bao/shared/constants/http";
import { openaiCompatRoutes } from "./openai-compat.routes";

const createCompatApp = () => new Elysia().use(openaiCompatRoutes);

describe("openai compatible routes", () => {
  test("GET /v1/models returns OpenAI list envelope", async () => {
    const app = createCompatApp();
    const response = await app.handle(new Request(`http://localhost${OPENAI_COMPAT_ENDPOINTS.models}`));
  ...
  test("GET /v1/models/* returns 404 for unknown model", async () => {
    ...
    const response = await app.handle(
      new Request(`http://localhost${OPENAI_COMPAT_ENDPOINTS.models}/local%2Fmissing-model`),
    );
    const raw = await response.text();
  ...
  test("POST /v1/chat/completions returns OpenAI chat.completion shape", async () => {
    ...
    const modelsResponse = await app.handle(
      new Request(`http://localhost${OPENAI_COMPAT_ENDPOINTS.models}`),
    );
    ...
    const response = await app.handle(
      new Request(`http://localhost${OPENAI_COMPAT_ENDPOINTS.chatCompletions}`, {
        method: "POST",
