...
/**
 * Canonical daisyUI stats shell variants. Every stats row in the app must
 * reference one of these — no per-page `stats stats-*` class chains.
 */
export const STATS_SHELL_VARIANT_CLASS = {
  /** Border + bg-base-200, horizontal @sm. StatsRow + LoadingSkeleton(stats). */
  default: "stats stats-vertical w-full border border-base-300 bg-base-200 sm:stats-horizontal",
  /** bg-base-200, horizontal @lg. Most common in-page stats. */
  lg: "stats stats-vertical w-full bg-base-200 lg:stats-horizontal",
  /** bg-base-200, horizontal @sm. Studios pages. */
  sm: "stats stats-vertical w-full bg-base-200 sm:stats-horizontal",
  /** Border + bg-base-100, horizontal @lg. Cover-letter / detail surfaces. */
  surfaceLg: "stats stats-vertical w-full border border-base-300 bg-base-100 lg:stats-horizontal",
  /** Border + bg-base-200, horizontal @xl. Scraper capability stats. */
  xl: "stats stats-vertical w-full border border-base-300 bg-base-200 xl:stats-horizontal",
  /** Vertical only, bg-base-100. Preferred provider card. */
  vertical: "stats stats-vertical w-full bg-base-100",
  /** Border + bg-base-100, sm→horizontal, xl→vertical. Brand stats card. */
  brandResponsive:
...
/**
 * Canonical daisyUI progress bar color variants. The base `progress` class
 * stays static on the element; these tokens supply the semantic color only.
 * No per-page `progress-primary` / `progress-success` literals allowed.
 */
...
/**
 * Canonical daisyUI alert color variants. The base `alert` class stays
 * static on the element; these tokens supply the semantic color only.
 * No per-page `'alert-success'` / `'alert-error'` literals in :class.
 */
