const gamificationPage = {
  gamificationPage: {
    pageTitle: "Gamification Hub",
    metricsSummary: "{brand} progression and challenge engine",
    seoTitle: "{brand} Gamification Hub",
    seoDescription:
      "Track XP progression, daily challenge completion, streak consistency, and achievement unlocks in one view.",
    loadErrorFallback: "Failed to load gamification data",
    retryButtonLabel: "Retry",
    retryAria: "Retry loading gamification data",
    emptyStateTitle: "No progression data yet",
    emptyStateDescription:
      "Complete setup tasks, start interview practice, and take daily challenges to begin earning XP and unlocking achievements.",
    emptyStateCta: "Open Dashboard",
    levelPrefix: "Level",
    xpSuffix: "XP",
    xpUntilLevelLabel: "XP until level",
    xpMaxLevelLabel: "Max level reached — every action still earns XP",
    currentStreakTitle: "Current Streak",
    longestStreakTitle: "Longest Streak",
    achievementsTitle: "Achievements",
    achievementsUnlockedLabel: "Unlocked",
    achievementsLockedLabel: "Locked",
    dailyChallengesTitle: "Daily Challenges",
    challengeClaimLabel: "Claim Reward",
    challengeClaimAria: "Claim reward for challenge {challenge}",
    challengeDoneLabel: "Done",
    challengeCompletionToast: "Challenge completed",
    challengeCompleteErrorFallback: "Failed to complete challenge",
    levelUpToast: "Level up! You reached level {level} — {title}",
    streakDaysSuffix: "days in a row",
    longestStreakDesc: "personal best",
    noChallengesLabel: "No daily challenges available.",
    noChallengesTitle: "No daily challenges available",
    noChallengesDescription:
      "Complete setup and career actions from the dashboard to unlock today's challenge set.",
    a11y: {
      levelProgress: "Level progression",
      challengeProgress: "Challenge progression",
    },
    achievementBadgeAria: "Achievement: {name}. {description}",
    trends: {
      title: "Progress trends",
      description: "Weekly momentum and monthly career stats from live gamification fabric.",
      weeklyTitle: "This week",
      weeklyXp: "{xp} XP earned",
      weeklyActions: "{count} actions logged",
      weeklyChallenges: "{count} challenges completed",
      weeklyEmpty: "No weekly activity yet — complete a challenge to start the trend.",
      topCategory: "Top category: {category}",
      monthlyTitle: "This month",
      monthlyXp: "{xp} total XP",
      monthlyLevels: "{count} levels gained",
      monthlyAchievements: "{count} achievements unlocked",
      monthlyStreak: "{days}-day streak",
      monthlyEmpty: "No monthly stats yet — keep practicing to fill this view.",
    },
    activity: {
      title: "Weekly activity & career progress",
      description: "Actions logged over the last seven days plus long-range career metrics.",
      weeklyTitle: "This week",
      weeklyTotalXp: "{xp} XP earned this week",
      daySummary: "{count} actions · {xp} XP",
      weeklyEmpty: "No actions logged in the last seven days.",
      careerTitle: "Career progress",
      skillCoverage: "Skill coverage: {value}%",
      applicationSuccessRate: "Application success rate: {value}%",
      interviewSessions: "{count} interview sessions tracked",
      careerEmpty: "No career progress metrics yet.",
      emptyTitle: "No activity tracked yet",
      emptyDescription:
        "Search jobs, save roles, and complete challenges to start building weekly activity and career progress.",
      emptyCta: "Browse jobs",
      emptyCtaAria: "Open the job board to start tracking activity",
      errorTitle: "Activity stats unavailable",
      retryButton: "Retry",
      retryAria: "Retry loading activity stats",
    },
  },
} as const;

export default gamificationPage;
