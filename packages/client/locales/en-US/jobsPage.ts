const jobsPage = {
  jobsPage: {
    seoTitle: "Job Board",
    seoDescription:
      "Search and filter game-industry roles by platform, studio type, genre, and experience level.",
    title: "Job Board",
    emptyStateTitle: "No jobs match your filters",
    emptyStateDescription:
      "Adjust your search query or filters to widen the current job discovery view.",
    emptyCatalogTitle: "No jobs loaded yet",
    emptyCatalogDescription:
      "Refresh the job feed or configure job providers in Settings to start discovery.",
    configureProvidersButton: "Configure job providers",
    configureProvidersAria: "Open Settings to configure job providers",
    refreshButton: "Refresh Jobs",
    refreshAria: "Refresh job feed",
    aiMatchButton: "AI Match",
    aiMatchAria: "Match jobs to your primary resume with AI",
    recommendationsTitle: "Recommended for you",
    recommendationsAria: "AI-recommended jobs",
    openRecommendationAria: "Open recommended job {title} at {company}",
    searchPlaceholder: "Search jobs…",
    searchAria: "Search jobs by title, company, or keywords",
    clearSearchAria: "Clear job search input",
    searchButton: "Search",
    searchButtonAria: "Run job search",
    toggleFiltersButton: "Filters",
    toggleFiltersAria: "Toggle filter panel",
    filtersTitle: "Filters",
    filtersActiveCount: "{count} active",
    applyFiltersButton: "Apply",
    applyFiltersAria: "Apply filters and close panel",
    clearFiltersButton: "Clear",
    clearFiltersAria: "Clear all job filters",
    locationLegend: "Location",
    locationPlaceholder: "City, State, or Country",
    locationAria: "Filter jobs by location",
    remoteOnlyLabel: "Remote Only",
    remoteOnlyAria: "Filter remote jobs only",
    experienceLegend: "Experience Level",
    experienceAria: "Filter by experience level",
    studioTypeLegend: "Studio Type",
    studioTypeAria: "Filter by studio type",
    platformLegend: "Platform",
    platformAria: "Filter by platform",
    genreLegend: "Genre",
    genreAria: "Filter by game genre",
    options: {
      all: "All",
      allTypes: "All Types",
      allPlatforms: "All Platforms",
      allGenres: "All Genres",
      experience: {
        entry: "Entry",
        junior: "Junior",
        mid: "Mid",
        senior: "Senior",
        principal: "Principal",
        director: "Director",
      },
      studioType: {
        aaa: "AAA",
        indie: "Indie",
        mobile: "Mobile",
        vrAr: "VR/AR",
        platform: "Platform",
        esports: "Esports",
        unknown: "Unknown",
      },
      platform: {
        pc: "PC",
        console: "Console",
        mobile: "Mobile",
        vr: "VR",
        ar: "AR",
        web: "Web",
        switch: "Switch",
        playStation: "PlayStation",
        xbox: "Xbox",
        steam: "Steam",
      },
      genre: {
        action: "Action",
        rpg: "RPG",
        strategy: "Strategy",
        puzzle: "Puzzle",
        simulation: "Simulation",
        sports: "Sports",
        racing: "Racing",
        shooter: "Shooter",
        platformer: "Platformer",
        horror: "Horror",
        mmorpg: "MMORPG",
        moba: "MOBA",
        battleRoyale: "Battle Royale",
        roguelike: "Roguelike",
        sandbox: "Sandbox",
        adventure: "Adventure",
        fighting: "Fighting",
        survival: "Survival",
        cardGame: "Card Game",
        casual: "Casual",
        indie: "Indie",
      },
    },
    bootstrapError: "Could not load the job board.",
    bootstrapRetry: "Retry",
    bootstrapRetryAria: "Retry loading the job board",
    tabs: {
      aria: "Job board views",
      browse: "Browse",
      browseAria: "Show job discovery results",
      saved: "Saved",
      savedAria: "Show saved jobs",
      applied: "Applied",
      appliedAria: "Show job applications",
    },
    saved: {
      emptyTitle: "No saved jobs yet",
      emptyDescription:
        "Save roles from the job board to build a shortlist for tailored resumes and interview prep.",
      emptyCta: "Browse jobs",
      emptyCtaAria: "Switch to the job discovery tab",
      errorTitle: "Could not load saved jobs.",
      retryButton: "Retry",
      retryAria: "Retry loading saved jobs",
      openSavedAria: "Open saved job {title} at {company}",
    },
    applied: {
      emptyTitle: "No applications yet",
      emptyDescription:
        "Apply to roles from the job board and track each application's status here.",
      emptyCta: "Browse jobs",
      emptyCtaAria: "Switch to the job discovery tab",
      errorTitle: "Could not load applications.",
      retryButton: "Retry",
      retryAria: "Retry loading applications",
      openApplicationAria: "Open application for {title} at {company}",
      appliedOn: "Applied {date}",
      status: {
        applied: "Applied",
        reviewing: "Reviewing",
        interviewing: "Interviewing",
        offered: "Offered",
        rejected: "Rejected",
        withdrawn: "Withdrawn",
      },
    },
    openJobAria: "Open job details for {title} at {company}",
    remoteBadge: "Remote",
    matchScoreAria: "Job match score",
    matchBreakdown: {
      overallProgressAria: "Overall match score: {score} percent",
      skillsMatchLabel: "Skills Match",
      skillsProgressAria: "Skills match progress",
      experienceMatchLabel: "Experience Match",
      experienceProgressAria: "Experience match progress",
      locationMatchLabel: "Location Match",
      locationProgressAria: "Location match progress",
    },
    interviewButton: "Interview",
    interviewAria: "Start interview for {title} at {company}",
    viewButton: "View",
    viewAria: "View job details for {title} at {company}",
    pagination: {
      navigationAria: "Job pagination",
      previousAria: "Previous job page",
      nextAria: "Next job page",
      pageAria: "Go to job page {page}",
      summary: "Showing {start}-{end} of {total} jobs",
    },
    previousPageAria: "Previous page",
    nextPageAria: "Next page",
    pageAria: "Go to page {page}",
    toasts: {
      searchReward: "Search progress recorded (+{xp} XP)",
      matchNeedsResume: "Create a resume before running AI job matching.",
      matchComplete: "AI job matching complete. Recommendations updated.",
    },
    date: {
      unknown: "Unknown",
      today: "Today",
      yesterday: "Yesterday",
      daysAgo: "{count} days ago",
      weeksAgo: "{count} weeks ago",
      monthsAgo: "{count} months ago",
    },
  },
} as const;

export default jobsPage;
