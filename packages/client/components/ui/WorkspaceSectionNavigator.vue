      ...
      <div class="card-body max- overflow-x-clip lg:p-5" :class="[FLUID_WIDTH_CLASS, TRUNCATE_FLEX_CHILD_CLASS, FLEX_GAP_TOKEN_CLASS.gap4, PADDING_TOKEN_CLASS.p4]">
        ...
        >
          <div class="flex items-start" :class="[TRUNCATE_FLEX_CHILD_CLASS, FLEX_GAP_TOKEN_CLASS.gap3]">
            ...
            >
              <span class="inline-flex items-center justify-center border border-primary/30 bg-primary/10 text-primary" :class="[SHADOW_TOKEN_CLASS.sm, RADIUS_TOKEN_CLASS.full, ICON_SIZE_CLASS[10]]">
                <component :class="[ICON_SIZE_CLASS[5]]" :is="resolveAppIconComponent(activeSectionEntry.iconName)" aria-hidden="true"/>
              </span>
            </span>
          ...
          >
            <div class="tabs tabs-box w-max min- glass-subtle xl:" :class="[FLUID_WIDTH_CLASS, TRUNCATE_FLEX_CHILD_CLASS, FLEX_GAP_TOKEN_CLASS.gap2, PADDING_TOKEN_CLASS.p2]">
              <NuxtLink class="tab h-auto grow justify-start rounded-box text-left xl:grow-0" v-for="section in sections" :key="section.id" :to="buildRoute(section.id)" :class="[FLEX_GAP_TOKEN_CLASS.gap3, activeSection === section.id ? 'tab-active' : '', MIN_HEIGHT_ZERO_CLASS, PADDING_TOKEN_CLASS.px3, PADDING_TOKEN_CLASS.py2]" :aria-current="activeSection === section.id ? 'page' : undefined">
                <span 
                  class="tooltip tooltip-bottom shrink-0"
                  :data-tip="section.descriptionKey ? t(section.descriptionKey) : t(section.labelKey)"
                >
                  <span class="inline-flex items-center justify-center border" :class="[SHADOW_TOKEN_CLASS.sm, RADIUS_TOKEN_CLASS.full, activeSection === section.id ? 'border-primary/30 bg-primary/10 text-primary' : 'border-base-300 bg-base-100 text-muted', ICON_SIZE_CLASS[8]]">
                    <component :class="[ICON_SIZE_CLASS[4]]" :is="resolveAppIconComponent(section.iconName)" aria-hidden="true"/>
                  </span>
                </span>
