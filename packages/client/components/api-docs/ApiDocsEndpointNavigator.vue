          ...
          <li v-for="group in endpointGroups" :key="group.id" :class="[STACK_SPACE_Y_TOKEN_CLASS.stack2]">
            ...
            <ul :class="[STACK_SPACE_Y_TOKEN_CLASS.stack2]">
              <li v-for="endpoint in group.endpoints" :key="endpoint.id">
                <button class="h-auto justify-start whitespace-normal text-start" type="button" :class="[GHOST_ACTION_DENSE_CLASS, TOUCH_TARGET_MIN_CLASS, FLUID_WIDTH_CLASS, { [PRIMARY_BUTTON_VARIANT_CLASS]: activeEndpointId === endpoint.id }, PADDING_TOKEN_CLASS.py2]" :aria-label=" t('apiDocs.endpoint.navigateAria', { method: methodLabel(endpoint.method), path: endpoint.path, }) " :aria-current="activeEndpointId === endpoint.id ? 'location' : undefined" @click="emit('navigate', endpoint.id)">
