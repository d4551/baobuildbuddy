...
// Blocking: settings + auth must succeed. Dashboard stats are optional (null on failure).
const {
  status: setupBootstrapStatus,
  error: setupBootstrapError,
  refresh: refreshSetupBootstrap,
} = await useAsyncData(
  "setup-bootstrap",
  async () => {
...
const setupBootstrapPending = computed(
  () => setupBootstrapStatus.value === "pending" || setupBootstrapStatus.value === "idle",
);
