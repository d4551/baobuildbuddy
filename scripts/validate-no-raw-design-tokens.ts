...
const rawMonochromePalettePattern =
  /\b(?:bg|text|border|fill)-(?:white|black)\b/gu;
...
const inlineShadowTokenPattern =
  /\bshadow-(?:xs|sm|md|lg|xl|2xl|inner|none)\b/gu;
const inlineRadiusTokenPattern =
  /\brounded-(?:sm|md|lg|xl|2xl|3xl|full)\b/gu;
  ...
  for (const classMatch of content.matchAll(staticClassAttributePattern)) {
    ...
    for (const { pattern, message } of patterns) {
      ...
      for (const tokenMatch of classValue.matchAll(pattern)) {
        ...
        if (token.includes("text-base") || token.includes("text-muted") || token.includes("text-secondary") || token.includes("text-primary")) {
          continue;
        }
...
export const collectRawDesignTokenViolationsForContent = (
  ...
  const classViolations = templateContent.length > 0
    ? collectClassAttributeViolations(filePath, content)
    : [];
