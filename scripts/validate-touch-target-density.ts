...
const BTN_XS_PATTERN = /\bbtn-xs\b/u;
...
export const collectTouchTargetDensityViolations = (
  ...
  for (const match of content.matchAll(CLASS_ATTR_PATTERN)) {
    ...
    if (!BTN_XS_PATTERN.test(classValue)) {
      continue;
    }
    // Same attribute must also reference TOUCH_TARGET_MIN_CLASS (via :class binding nearby).
    ...
    if (TOUCH_FLOOR_PATTERN.test(nearby)) {
      continue;
    }
    violations.push({
      filePath,
      line: getLineFromOffset(content, attrStart),
      message: `btn-xs without TOUCH_TARGET_MIN_CLASS ("${classValue}"); use btn-sm + TOUCH_TARGET_MIN_CLASS`,
    });
  }
  return violations;
};
